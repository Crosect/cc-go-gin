...
func NewHTTPServer(app *ccgo.App, engine *gin.Engine) *http.Server {
	return &http.Server{
...
func RegisterHandlers(app *ccgo.App, engine *gin.Engine) {
	engine.Use(InitContext())
	engine.Use(WrapAll(app.Handlers())...)
}

func OnStartHttpServerHook(lc fx.Lifecycle, app *ccgo.App, httpServer *http.Server) {
	lc.Append(fx.Hook{
...
func StartOpt() fx.Option {
	return fx.Invoke(func(lc fx.Lifecycle, app *ccgo.App, engine *gin.Engine) {
		lc.Append(fx.Hook{
			OnStart: func(ctx context.Context) error {
